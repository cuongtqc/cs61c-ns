...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    livingGhostPositionDistributions = [beliefs for i,beliefs
                                        ...
                                        if livingGhosts[i+1]]
    "*** YOUR CODE HERE ***"
    likelyGhosts = util.Counter()
    
    for ghostPos in livingGhostPositionDistributions:
        likelyGhosts[ghostPos.argMax()] = -(self.distancer.getDistance(gameState.getPacmanPosition(), ghostPos.argMax()))
    destination = likelyGhosts.argMax()
    
    #want position for each ghost with highest probability, and sort these by distance
    likelyPositions = util.Counter()
    for action in gameState.getLegalActions():
        successorPosition = Actions.getSuccessor(gameState.getPacmanPosition(), action)
        likelyPositions[action] = -(self.distancer.getDistance(successorPosition, destination))
    #print likelyPositions.argMax()
    return likelyPositions.argMax()
